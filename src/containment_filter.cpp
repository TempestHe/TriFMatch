...
bool ContainmentFilter::validate_complete_containment(uint32_t depth, vector<vector<pair<Vertex*, uint32_t>>>& candidates_stack, Vertex root, CandidatesHistoryStack& recorder, vector<Vertex>& embedding_depth){
    // for(int offset=0;offset<recorder.history_root_vertices_.size();++offset){ // auto offset : conflict_record_offset[depth]
    //     if(recorder.validation_marker_[offset] == 0){
    //         continue;
    //     }
        ...
        if(contained == true){
            ...
            if(contained == true){
                ...
                for(auto d : candidates_containment_check){
                    ...
                    // if(validate_vector_containment_relaxed_full(&(v2_neighbor[0]), v2_neighbor.size(), &(v1_neighbor[0]), v1_neighbor.size(), history_root, root) == false){
                    //     contained = false;
                    //     break;
                    // }
