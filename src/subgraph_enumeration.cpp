...
SubgraphEnum::SubgraphEnum(Graph* data_graph){
    ...
    successor_equivalent_sets_ = NULL;
#endif
}
...
void SubgraphEnum::match(Graph* query_graph, string ordering_method, uint64_t count_limit, uint32_t time_limit){
    ...
    for(int d=1;d<=query_vertex_count_;++d){
        history_candidates_[d].rebuild(query_graph_, data_graph_, successor_neighbors_in_depth_[d], d);
    }
    ...
    while(true){
        while(candidates_offset_[cur_depth]<searching_candidates_[cur_depth].size()){
            ...
            max_search_depth[cur_depth] = cur_depth;

            // if(cur_depth==1){
            //     cout<<"cur:"<<v<<":"<<candidates_offset_[cur_depth]<<endl;
            ...
            if(cur_depth == query_vertex_count_){
                ...
                // start intersection
                history_candidates_[cur_depth].append_history(v, encoder, candidates_stack_);
...
#if SUCCESSOR_EQUIVALENT_SET == 1
#if PRINT_RESULT == 1
                        successor_equivalent_sets_->recording_matches(&(embedding_depth_[0]), emb_count_, cur_depth, matches_);
#endif
                        successor_equivalent_sets_->stop_recording(&(embedding_depth_[0]), &cur_sets, emb_count_, cur_depth);
#endif
                        max_search_depth[cur_depth-1] = (max_search_depth[cur_depth-1]<max_search_depth[cur_depth]) ? max_search_depth[cur_depth] : max_search_depth[cur_depth-1];
                        continue;
                    }
...
EXIT:
    ...
    storage_ = NULL;
#if SUCCESSOR_EQUIVALENT_SET == 1
    delete successor_equivalent_sets_;
    successor_equivalent_sets_ = NULL;
#endif
}
