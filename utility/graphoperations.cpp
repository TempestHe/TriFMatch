...
void GraphOperations::getKCore(const Graph *graph, int *core_table) {
    ...
    for (int i = 0; i < vertices_count; ++i) {
        ...
        uint32_t count;
        const Vertex * neighbors = graph->getVertexNeighbors(v, count);
...
void GraphOperations::dfs(TreeNode *tree, Vertex cur_vertex, Vertex *dfs_order, uint32_t &count) {
    dfs_order[count++] = cur_vertex;

    for (uint32_t i = 0; i < tree[cur_vertex].children_count_; ++i) {
